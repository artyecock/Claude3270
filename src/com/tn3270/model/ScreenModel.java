...
public class ScreenModel {
    ...
    // Defaults for standard 3270 colors (0=Black, 1=Blue... 7=White)
    private Color[] palette = {
        Color.BLACK, Color.BLUE, Color.RED, Color.MAGENTA, Color.GREEN, Color.CYAN, Color.YELLOW, Color.WHITE 
    };
    ...
    public ScreenModel(String modelName, Map<String, Dimension> models) {
        ...
        if (dim == null) dim = new Dimension(80, 24);

        // Logic for Primary/Alternate sizes
        this.primaryCols = 80;
        this.primaryRows = 24;
        this.alternateCols = dim.width;
        this.alternateRows = dim.height;
        
        // Start in alternate mode usually
    ...
    public void resetMDT() {
        for(int i=0; i<attributes.length; i++) attributes[i] &= ~0x01;
    }
    
    // --- Data Accessors ---
    public int getSize() { return buffer.length; }
    public char getChar(int i) { return (i>=0 && i<buffer.length) ? buffer[i] : '\0'; }
    public void setChar(int i, char c) { if (i>=0 && i<buffer.length) buffer[i] = c; }
    
    public byte getAttr(int i) { return (i>=0 && i<attributes.length) ? attributes[i] : 0; }
    public void setAttr(int i, byte b) { if (i>=0 && i<attributes.length) attributes[i] = b; }

    public void setExtendedColor(int i, byte b) { if (i>=0 && i<extendedColors.length) extendedColors[i] = b; }
    public void setHighlight(int i, byte b) { if (i>=0 && i<highlighting.length) highlighting[i] = b; }
    ...
    public byte[] getHighlight() { return highlighting; }
    ...
    public void setUseAlternateSize(boolean b) {
        ...
        if (b) { rows = alternateRows; cols = alternateCols; }
        else   { rows = primaryRows;   cols = primaryCols; }
    }
    
    // --- Palette & Colors ---
    public void setPalette(Color[] colors) { 
        // Ensure we copy or reference safely
    ...
    // --- Field Logic Helpers ---
    public boolean isFieldStart(int pos) { return attributes[pos] != 0; }
    
    public boolean isProtected(int pos) {
        int start = findFieldStart(pos);
        if (start == -1) return false; // Unformatted
    ...
    public void setModified(int pos) {
        int start = findFieldStart(pos);
        if (start != -1) attributes[start] |= 0x01;
    }
    
    public boolean isKeyboardLocked() { return keyboardLocked; }
    public void setKeyboardLocked(boolean b) { this.keyboardLocked = b; }
