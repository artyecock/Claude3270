...
public class AIManager implements AIModelProvider {
    ...
    private static class DetectionRule {
        ...
        boolean matches(String host, String filename, String content) {
            ...
            for (String trig : fileTriggers) {
                if (fnUpper.contains(trig)) return true;
            }

            // 3. Check Text Triggers (OR logic)
            for (String trig : textTriggers) {
                if (txtUpper.contains(trig)) return true;
            }
    ...
    public String buildAugmentedPrompt(String filename, String content, String hostType) {
        ...
        if (matchedKey != null) {
            ...
            if (prompt != null) {
                ...
                if (ref != null) {
                    sb.append("Reference Material: Please base answer on definitions found in '")
                      .append(ref).append("'");
                    if (url != null) sb.append(" (Available at: ").append(url).append(")");
                    sb.append(".\n");
                }
                sb.append("\n");
            }
    ...
    public String buildAugmentedPromptOld(String filename, String content, String hostType) {
        ...
        if (hostType.contains("CMS") || hostType.contains("VM")) {
            if (fn.contains(" EXEC") || content.contains("/* REXX */")) key = "cms_rexx";
            else if (fn.contains(" ASSEMBLE")) key = "asm";
            else if (content.contains("PIPE")) key = "pipelines";
        } else { // TSO
            if (content.contains("/* REXX */")) key = "tso_rexx";
            else if (content.contains("//") && content.contains("JOB")) key = "jcl";
            else if (fn.contains("COBOL") || content.contains("IDENTIFICATION DIVISION")) key = "cobol";
        }
    ...
    /*
    private String buildAugmentedPromptNew(String filename, String content, TN3270Session.HostType hostType) {
        StringBuilder sb = new StringBuilder();
        AIConfig config = AIConfig.getInstance();
        
        // 1. Determine Context Keys
        String fileExt = getExtension(filename); // e.g., "EXEC", "JCL", "ASSEMBLE"
        
        // 2. Select Primer based on Logic
        String primer = "";
        
        if (hostType == TN3270Session.HostType.CMS) {
            if (content.contains("// REXX //") || filename.contains(" EXEC ")) {
                primer = config.getContext("cms_rexx");
            } else if (filename.contains(" ASSEMBLE ")) {
                primer = config.getContext("asm");
            } else if (content.contains("PIPE")) {
                primer = config.getContext("pipelines");
            }
        } else { // TSO
            if (content.contains("// REXX //")) {
                primer = config.getContext("tso_rexx");
            } else if (content.contains("//") && content.contains("JOB")) {
                primer = config.getContext("jcl");
            }
        }
