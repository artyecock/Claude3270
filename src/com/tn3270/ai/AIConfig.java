...
public class AIConfig {
	private static final Logger logger = LoggerSetup.getLogger(AIConfig.class);
	...
	public void load() {
		try {
			...
			logger.info("Loading AI Config from: " + file.getAbsolutePath());

			if (file.exists()) {
				try (FileInputStream fis = new FileInputStream(file)) {
					props.load(fis);
					//System.out.println("Config loaded successfully. Keys found: " + props.keySet().size());
					logger.info("Config loaded successfully. Keys found: " + props.keySet().size());
				}
			} else {
				//System.out.println("!!! Config file NOT found at that path. Using defaults.");
				logger.info("!!! Config file NOT found at that path. Using defaults.");
				createDefaults();
			}
		} catch (Exception e) {
			//System.err.println("AIConfig.load: " + e.getMessage());
			logger.severe("AIConfig.load: " + e.getMessage());
		}
	}
