...
public class TerminalPanel extends JPanel implements Scrollable {
	...
	// Border color for scroll-back mode indication
	private Color borderColor = Color.GREEN;
	private boolean showBorder = false;  // Only show border when debug mode is active
	...
	/**
	 * Enable or disable the debug border.
	 * Border only shows when debug mode is active.
	 * 
	 * @param show true to show border, false to hide
	 */
	...
	/**
	 * Set the border color for the terminal panel. Used to indicate scroll-back
	 * mode (red) vs. live mode (green).
	 * 
	...
	protected void paintComponent(Graphics g) {
		...
		for (int row = 0; row < rows; row++) {
			for (int col = 0; col < cols; col++) {
				 ...
				 * color. Default color shows characters in green unless field attributes
				 * specify bright intensity, in which case they are white. In base color mode,
				 * the protection and intensity bits are used in combination to select among
				 * four colors: normally white, red, blue, and green; the protection bits retain
				 * their protection functions as well as determining color. (If you use extended
				 * color, rather than base color, for 3270 terminals, note that you cannot
				 * specify "white" as a color. You need to specify "neutral", which is displayed
				 * as white on a terminal.)
				 * 
				 */
				/*
				 * if (charColor > 0 && charColor < palette.length) { // Character has explicit
				 * SA color - USE IT, no overrides based on basic attributes fg =
				 * palette[charColor];
				 * 
				 * } else if (fieldStart >= 0) { int fieldColor = extColors[fieldStart] & 0xFF;
				 * if (fieldColor > 0 && fieldColor < palette.length) { // Field has explicit
				 * color - USE IT fg = palette[fieldColor]; } else { // No explicit colors - use
				 * basic attribute to determine color boolean prot = (a & 0x20) != 0; boolean
				 * high = (a & 0x08) != 0;
				 * 
				 * fg = high ? (prot ? palette[7] : palette[2]) : (prot ? palette[5] :
				 * palette[4]); } }
				 */
				if (!isHidden) {
					// x20 = protected
					...
					if (charColor > 0 && charColor < palette.length) {
						...
						if (fieldColor > 0 && fieldColor < palette.length) {
							...
							if (fieldColor == 1 && prot && numeric && isVerticalBar) {
								...
								// System.out.println(String.format("Forcing char '%c' at pos=%d num=%s prot=%s
								// high=%s", c, pos, numeric, prot, high));
							} else {
					...
					// System.out.println("Pos: " + pos + ", Color attribute: " + charColor + ",
					// Rendered color: " + fg + " (Base color " + baseFg +")");
					// System.out.println(String.format("Writing char '%c' at pos=%d Color
					// attribute: %s Rendered color: %s (Base color: %s)", c, pos, charColor, fg,
					// baseFg));
					// Handle highlighting
					...
					/*
					 * if (c == '¦' && row >= 5 && row <= 12) { System.out.println(String.format(
					 * "Bar at pos %d (row %d, col %d): charColor=%d fieldStart=%d fieldColor=%d prot=%b numeric=%b high=%b finalColor=%s"
					 * , pos, row, col, charColor, fieldStart, (fieldStart >= 0 ?
					 * extColors[fieldStart] & 0xFF : -1), (a & 0x20) != 0, (a & 0x10) != 0, (a &
					 * 0x08) != 0, fg.toString() )); }
					 */
				}
		...
		// === DRAW BORDER (only when debug mode is active) ===
	    if (showBorder) {
	    	drawBorder(g);
	    }
	}
	...
	/**
	 * Draw a rounded border around the terminal. Border color changes based on
	 * mode: GREEN = live, RED = scroll-back.
	 */
	private void drawBorder(Graphics g) {
		...
		try {
			...
			g2d.drawRoundRect(inset, inset, getWidth() - borderWidth - 1, getHeight() - borderWidth - 1, radius,
					radius);
		} finally {
			g2d.dispose();
		}
	...
	protected void paintComponentWorks(Graphics g) {
		...
		for (int row = 0; row < rows; row++) {
			for (int col = 0; col < cols; col++) {
				...
				// -----------------------------------------------------------------------------
				// Problem: The 3270 "Blue" color (0xF1) is often illegible on black
				// backgrounds.
				// Host applications (like CMS/ISPF) often define "Protected" fields (labels)
				// with "Blue" extended color, but also set the "High Intensity" basic
				// attribute.
				//
				// Architecture vs. Reality:
				// - 3270 Arch (GA23-0059): Extended Color (Blue) takes precedence. Result: Dark
				// Blue.
				// - IBM PCOMM Behavior: Detects "Protected + Blue" (and often High Intensity)
				// and dynamically remaps it to Yellow (or White) to ensure the text is
				// readable.
				...
				if (c == '|') { // Just display vertical bars as "tall", no matter what
					/*
					 * // OPTION A: Swap to Unicode Box Drawing char // (Simplest, assumes font
					 * supports U+2502) // c = '\u2502';
					 * 
					 * // OPTION B: Manual Drawing (If you want "Continuous" lines) // If you want
					 * lines to touch exactly, fonts often leave gaps. // You can draw a line
					 * manually:
					 * 
					 * // int cx = col * charWidth; // int cy = row * charHeight; g2d.setColor(fg);
					 * // Use current foreground
					 * 
					 * // FIX: Use 'x' and 'y' (which include margins), not raw col/row calcs int
					 * centerX = x + (charWidth / 2);
					 * 
					 * // Draw line down the exact center, full height g2d.drawLine(centerX, y,
					 * centerX, y + charHeight);
					 * 
					 * // Draw line down the exact center // g2d.drawLine(cx + charWidth/2, cy, cx +
					 * charWidth/2, cy + charHeight); continue; // Skip standard string drawing
					 */
					g2d.setColor(fg);
