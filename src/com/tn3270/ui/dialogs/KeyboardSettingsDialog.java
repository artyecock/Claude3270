...
public class KeyboardSettingsDialog extends JDialog {
    ...
    private JScrollPane create3270Panel() {
        ...
        for (int i = 1; i <= 12; i++) {
            add3270Button(panel3270, "PF" + i, PF_AID[i - 1], x, y, KW, KH);
            x += KW + GAP;
            if (i % 4 == 0) x += (GAP * 2);
        }
        ...
        add3270Button(panel3270, "ATTN", (byte) 0x6A, controlX, row3Y, 80, KH);

        y += KH + GAP; int row4Y = y; x = START_X;
        add3270Button(panel3270, "Tab", (byte) 0, x, y, (int) (KW * 1.5), KH); x += (int) (KW * 1.5) + GAP;
        ...
        add3270Button(panel3270, "SYS REQ", (byte) 0xF0, controlX, row4Y, 80, KH);
    ...
    private void add3270Button(JPanel p, String label, byte aidCode, int x, int y, int w, int h) {
        ...
        btn.addActionListener(e -> {
            ...
            if (cap.cleared) {
                ...
                KeyMapping newMap;
                if (label.equals("RESET")) newMap = new KeyMapping((byte)0, "RESET");
                else if (label.equals("EraseEOF")) newMap = new KeyMapping((byte)0, "EraseEOF");
                else if (label.equals("NEWLINE") || label.equals("R-Shift")) newMap = new KeyMapping(AID_ENTER, "NEWLINE");
                else if (label.equals("Back")) newMap = new KeyMapping((byte)0, "BACKSPACE");
                else if (label.equals("Tab")) newMap = new KeyMapping((byte)0, "TAB");
        ...
        for (JButton btn : buttons3270) {
            ...
            if (id.startsWith("3270CHAR:")) {
                ...
                for (Map.Entry<Integer, KeyMapping> entry : tempKeyMap.entrySet()) {
                    ...
                    if ((km.description != null && km.description.contains(search)) || (km.aid != null && km.aid == aid && aid != 0)) {
                        mapped = true; mappedTo = KeyEvent.getKeyText(entry.getKey()); break;
                    }
                }
            }
    ...
    private byte getAidForName(String name) {
        if (name.startsWith("PF")) { try { return PF_AID[Integer.parseInt(name.substring(2)) - 1]; } catch (Exception e) {} }
        ...
        if (name.equals("ATTN")) return (byte) 0x6A;
        if (name.equals("SYS REQ")) return (byte) 0xF0;
        return 0;
    }
    ...
    private void removeMappingsForFunction(String label, byte aid) {
        ...
        tempKeyMap.entrySet().removeIf(entry -> {
            ...
            if (km.aid != null && km.aid == aid && aid != 0) return true;
            return false;
        });
    }
