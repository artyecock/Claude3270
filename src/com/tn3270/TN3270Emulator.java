...
public class TN3270Emulator extends JFrame {
    ...
    public static class ConnectionProfile {
        ...
        public ConnectionProfile(String name, String hostname, int port, String model, String luName, boolean useTLS) {
            this.name = name; this.hostname = hostname; this.port = port;
            this.model = model; this.luName = luName; this.useTLS = useTLS;
        }
        @Override public String toString() { return name + " (" + hostname + ":" + port + ")"; }
    }
    ...
    public TN3270Emulator(String initialModel) {
        ...
        if (initialModel != null) {
            if (initialModel.trim().isEmpty()) initialModel = "3279-3";
        ...
        pack();
        if (getWidth() < 1100) setSize(1100, 850);
    ...
    public void openNewSession(String title, String h, int p, String m, String l, boolean t) {
        ...
        // Set initial flag based on current mode
    ...
    public void popInAllSessions() {
        TN3270Emulator target = findBestTargetWindow();
        if (target == null) {
            JOptionPane.showMessageDialog(this, "No other Main Window found to merge into.", "Pop-in Failed", JOptionPane.WARNING_MESSAGE);
            return;
        }
    ...
    public void closeSession(TN3270Session session) {
        ...
        if (activeSessions.isEmpty()) {
            ...
            SwingUtilities.invokeLater(() -> {
                TN3270Session current = getCurrentSession();
                if (current != null) current.snapWindow();
            });
        }
    }

    public void closeSessionOld(TN3270Session session) {
        if (session == null) return;
        session.disconnect();
        activeSessions.remove(session);
        if (activeSessions.isEmpty()) {
    ...
    private void closeAllSessionsAndExit() {
        ...
        for (Frame f : Frame.getFrames()) {
            if (f.isVisible()) { anyVisible = true; break; }
        }
    ...
    public TN3270Session getCurrentSession() {
        ...
        for (TN3270Session s : activeSessions) {
            if (s.isFocusOwner() || s.terminalPanel.isFocusOwner()) return s;
        }
    ...
    public void setViewMode(ViewMode mode) {
        ...
        if (menuViewTabs != null && menuViewTiles != null) {
            if (mode == ViewMode.TABS) menuViewTabs.setSelected(true);
            else menuViewTiles.setSelected(true);
        }
    ...
    // -----------------------------------------------------------------------
    // NEW: Auto-Grow Logic for Tiles
    
    /**
     * Adjusts the window size to accommodate multiple tiled sessions.
     * 
     * REGRESSION NOTE: "CRUSHED" WINDOWS
     * When switching from Tabs to Tiles, or adding a new Tile, the existing window size
     * is often too small to show multiple sessions, resulting in tiny, unreadable fonts.
     * 
     * Strategy:
     * 1. "Prime" all sessions by forcing them to a target font (e.g., 14pt) via 'setFontSizeNoResize'.
     * 2. Call 'pack()'. The Layout Manager calculates the total space needed for X tiles at 14pt.
     * 3. The Window grows to this new size.
     * 4. Finally, we re-enable 'AutoFitOnResize' so subsequent user drags scale the fonts dynamically.
     */
    private void snapWindowToTiles() {
        if (activeSessions.isEmpty()) return;
        // Prime sessions with legible font
        for (TN3270Session s : activeSessions) {
            s.setFontSizeNoResize(14); 
        }
        pack(); // Layout manager expands window to fit all 14pt tiles
        ...
        // Optional: Center if we are still within bounds
        if (w < maxW && h < maxH) {
             setLocationRelativeTo(null);
        }
    }
    
    private void snapWindowToTilesOld() {
        if (activeSessions.isEmpty()) return;

        // 1. Prime all sessions to a legible "Target Font" (e.g. 14pt)
        //    This sets the PreferredSize of every tile.
        for (TN3270Session s : activeSessions) {
            s.setFontSizeNoResize(14); 
        }

        // 2. Pack the window
        //    The GridLayout will sum up the PreferredSizes of all tiles
        //    and calculate the exact window size needed to show them all at 14pt.
        pack();

        // 3. Clamp to Screen Size (Safety)
        GraphicsConfiguration gc = getGraphicsConfiguration();
        Rectangle bounds = gc.getBounds();
        Insets insets = Toolkit.getDefaultToolkit().getScreenInsets(gc);
    ...
    private void updateViewLayout() {
        ...
        if (currentViewMode == ViewMode.TABS) {
            ...
            for (TN3270Session s : activeSessions) s.setAutoFitOnResize(false);
            
            SwingUtilities.invokeLater(() -> {
                TN3270Session s = getCurrentSession();
                if (s != null) s.snapWindow();
            });
        } else {
            renderTiles();
            // TILES MODE: AutoFit = TRUE (Resize Window -> Scale Fonts)
            for (TN3270Session s : activeSessions) s.setAutoFitOnResize(true);
    ...
    private void updateViewLayoutPrior() {
        mainContentPanel.removeAll();
        if (currentViewMode == ViewMode.TABS) {
            renderTabs();
            // Existing logic: Snap window to the single visible tab
            SwingUtilities.invokeLater(() -> {
                TN3270Session s = getCurrentSession();
                if (s != null) s.snapWindow();
            });
            ...
            renderTiles();
            // NEW: Auto-Grow window to fit all tiles comfortably
            SwingUtilities.invokeLater(this::snapWindowToTiles);
        }
    ...
    private void updateViewLayoutOld() {
        mainContentPanel.removeAll();
        if (currentViewMode == ViewMode.TABS) renderTabs();
        else renderTiles();
        mainContentPanel.revalidate();
        mainContentPanel.repaint();
    }
    ...
    private boolean hasOtherWindows() {
        int count = 0;
        for (Frame f : Frame.getFrames()) {
            if (f instanceof TN3270Emulator && f.isVisible()) count++;
        }
        return count > 1;
    }
    ...
    private void renderTiles() {
        int count = activeSessions.size(); if (count == 0) return;
        ...
        for (TN3270Session session : activeSessions) {
            ...
            // CRITICAL FIX: Add listener to the button container so clicks BETWEEN buttons work
            ...
            if (session.terminalPanel.isFocusOwner()) focusHighlighter.focusGained(null);
    ...
    private void renderTilesOld() {
        ...
        for (TN3270Session session : activeSessions) {
            ...
            MouseAdapter popOutAdapter = new MouseAdapter() {
                public void mouseClicked(MouseEvent e) { if (e.getClickCount() == 2) popOutSession(session); }
            };
            header.addMouseListener(popOutAdapter);
            lblTitle.addMouseListener(popOutAdapter);
            ...
            // 2. Tabify Button
            JButton btnTabify = createHeaderButton("≡", "Switch to Tabbed View");
            btnTabify.setFont(new Font("SansSerif", Font.BOLD, 14));
            btnTabify.addActionListener(e -> {
                setViewMode(ViewMode.TABS);
                selectSession(session);
    ...
    private JButton createHeaderButton(String text, String tooltip) {
        ...
        btn.addMouseListener(new MouseAdapter() {
            public void mouseEntered(MouseEvent e) { btn.setForeground(Color.DARK_GRAY); }
    ...
    private void createMenuBar() {
        ...
        JMenuItem copyItem = new JMenuItem("Copy"); copyItem.addActionListener(e -> copySelection()); editMenu.add(copyItem);
        JMenuItem pasteItem = new JMenuItem("Paste"); pasteItem.addActionListener(e -> pasteFromClipboard()); editMenu.add(pasteItem);
        editMenu.addSeparator();
        JMenuItem selectAllItem = new JMenuItem("Select All"); selectAllItem.addActionListener(e -> selectAll()); editMenu.add(selectAllItem);
        editMenu.addSeparator();
        ...
        ButtonGroup bg = new ButtonGroup(); bg.add(menuViewTabs); bg.add(menuViewTiles);
        viewMenu.add(menuViewTabs); viewMenu.add(menuViewTiles);
        ...
        viewMenu.addSeparator();
        JMenuItem fontSizeItem = new JMenuItem("Font Size..."); fontSizeItem.addActionListener(e -> showFontSizeDialog()); viewMenu.add(fontSizeItem);
        JMenuItem colorSchemeItem = new JMenuItem("Color Scheme..."); colorSchemeItem.addActionListener(e -> showColorSchemeDialog()); viewMenu.add(colorSchemeItem);
        menuBar.add(viewMenu);

        // --- SETTINGS ---
        JMenu settingsMenu = new JMenu("Settings");
        JMenuItem keyMapItem = new JMenuItem("Keyboard Mapping..."); keyMapItem.addActionListener(e -> showKeyboardMappingDialog()); settingsMenu.add(keyMapItem);
        JMenuItem termSetItem = new JMenuItem("Terminal Settings..."); termSetItem.addActionListener(e -> showTerminalSettingsDialog()); settingsMenu.add(termSetItem);
        menuBar.add(settingsMenu);

        // --- HELP ---
        JMenu helpMenu = new JMenu("Help");
        JMenuItem aboutItem = new JMenuItem("About"); aboutItem.addActionListener(e -> showAboutDialog()); helpMenu.add(aboutItem);
        JMenuItem keyboardHelpItem = new JMenuItem("Keyboard Reference"); keyboardHelpItem.addActionListener(e -> showKeyboardReference()); helpMenu.add(keyboardHelpItem);
    ...
    public static void showConnectionDialog(TN3270Emulator targetFrame) {
        ...
        JButton saveButton = new JButton("Save"); JButton deleteButton = new JButton("Delete");
        JButton connectButton = new JButton("Connect"); JButton cancelButton = new JButton("Cancel");
        ...
        dialog.pack(); dialog.setLocationRelativeTo(targetFrame); dialog.setVisible(true);
    }
    ...
    private static void loadProfiles() {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (parts.length >= 5) {
                    String name = parts[0]; String host = parts[1];
                    int port = 23; try { port = Integer.parseInt(parts[2]); } catch (Exception e) {}
                    String model = parts[3]; String luName = ""; boolean useTLS = false;
                    if (parts.length == 5) useTLS = Boolean.parseBoolean(parts[4]);
                    else if (parts.length >= 6) { luName = parts[4]; useTLS = Boolean.parseBoolean(parts[5]); }
                    savedProfiles.put(name, new ConnectionProfile(name, host, port, model, luName, useTLS));
                }
            }
        } catch (IOException e) { System.err.println("Error loading profiles: " + e.getMessage()); }
    }
    ...
    public static void saveProfiles() {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(PROFILES_FILE))) {
            for (ConnectionProfile p : savedProfiles.values()) {
                writer.write(p.name + "," + p.hostname + "," + p.port + "," + p.model + "," + (p.luName==null?"":p.luName) + "," + p.useTLS);
                writer.newLine();
            }
        } catch (IOException e) { System.err.println("Error saving profiles: " + e.getMessage()); }
    }
