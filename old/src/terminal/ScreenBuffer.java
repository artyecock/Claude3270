...
/**
 * Get attribute type for a position (for CHARACTER mode replies).
 * Returns the field attribute type (simplified implementation).
 */
public int getAttributeType(int pos) {
    // For now, return field attribute type
    // Can be extended to check for color/highlighting attributes
    return Constants.ATTR_FIELD;
}

/**
 * Get attribute value for a position (for CHARACTER mode replies).
 */
public int getAttributeValue(int pos) {
    int fieldStart = findFieldStart(pos);
    return getAttribute(fieldStart) & 0xFF;
}
